...
def load_data(
    ...
    if (continual == 're') or (continual ==  're+')or (continual ==  'h') or (continual ==  'gem'):
    ...
    # elif continual == 'ewc':
    #     train_set_aug = CarvanaDataset(augimg_t_dir, augmask_t_dir, img_scale)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            for mem_batch in memory_loader:
                ...
                for param, current_grad in zip(model.parameters(), current_grads):
                    if torch.dot(param.grad.reshape(-1), current_grad.reshape(-1)) < 0:
                        param.grad -= torch.dot(param.grad.reshape(-1), current_grad.reshape(-1)) / torch.norm(current_grad) ** 2 * current_grad        
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                ...
                loss = ewc_loss(model, fisher_information, optimal_params, ewc_lambda, base_loss, device)
...
class CustomClient(fl.client.NumPyClient):
    def __init__(
        ...
        self.memory_loader = memory_loader  # 用于GEM的内存数据
        self.model_dir = os.path.join(base_dir, f'./client_{args.num_client}/model')
    ...
    def fit(self, parameters, config):
        ...
        if (self.continual == 'ewc')or (self.continual == 'h'):
        ...
        elif (self.continual == 'gem'):
            info2server = train_model_gem(
                model=self.model,
                epochs=args.epochs,
                device=device,
                amp=args.amp,
                ...
                grad_scaler=self.grad_scaler,
                criterion=self.criterion,
                memory_loader= self.memory_loader,  # 用于GEM的内存数据
                val_loader_aug=self.test_aug,
...
def get_args():
    ...
    parser.add_argument('--continual', '-ct', type=str, default=False, help='Whether it is continuous training? (ewc/re/re+/h/gem)')
...
if __name__ == '__main__':
    ...
    # TODO: 需要修改目录到自己的数据集！
    data_base_dir = '/gris/gris-f/homelv/xsong/Final Projekt'
    
    if args.load:
        state_dict = torch.load(args.load, map_location=device)
        model.load_state_dict(state_dict)
        log(INFO, f'Model loaded from {args.load}')
        if (args.continual == 'ewc')or (args.continual == 'h'):
    ...
    if args.continual != 're+':

        trainloader, testloader, testloader_aug, num_examples, memory_loader= load_data(
            nc=args.num_client,
            nt=args.num_train,
            batch_size=args.batch_size,
        ...
        class CustomClient2(fl.client.NumPyClient):
            def __init__(
                self, 
                model,  
                lr, 
                ...
                # 在初始化时加载数据集，只加载一次
                self.trainloader, self.testloader, self.testloader_aug, self.num_examples, _ = load_data(
                    nc=args.num_client,
                    nt=args.num_train,
                    batch_size=args.batch_size,
                    img_scale=args.scale,
            ...
            def fit(self, parameters, config):
                ...
                info2server = train_model(
                    ...
                    alpha=args.alpha,
                    beta=args.beta,
                    num_examples=self.num_examples,  # 使用预加载的数据
                    train_loader=self.trainloader,  # 使用预加载的数据
                    val_loader=self.testloader,  # 使用预加载的数据
                    batch_size=args.batch_size,
                    optimizer=self.optimizer,
                    scheduler=self.scheduler,
                    grad_scaler=self.grad_scaler,
                    criterion=self.criterion,
                    val_loader_aug=self.testloader_aug,  # 使用预加载的数据
                )
                return self.get_parameters(config={}), self.num_examples["trainset"], info2server
            ...
            def evaluate(self, parameters, config):
                ...
                if (self.run_id % 10) == 0:
                    if self.save_model:
                        ...
                        log(INFO, f"Model's weights saved at round {self.run_id}")
                    if self.save_info:
                        fisher_information = compute_fisher_information(self.model, self.trainloader, self.criterion, device=device)
                        save_information(fisher_information, os.path.join(self.info_dir, f'fisher_t{args.num_train}.pkl'))
                        log(INFO, f'Fisher information saved at round {self.run_id}')
                        optimal_params = {name: param.clone() for name, param in self.model.named_parameters()}
                        save_information(optimal_params, os.path.join(self.info_dir, f'opt_t{args.num_train}.pkl'))
